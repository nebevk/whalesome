<template>
  <section class="py-20 px-6 ocean-depth-mid">
    <div class="max-w-6xl mx-auto">
      ...
      >
        Explore the diverse families and groups of these magnificent marine mammals
      </p>
        ...
        >
          <div class="card-body">
            <h3 class="card-title text-2xl text-white mb-2">
              {{ group.name }}
              <span class="text-sm font-normal text-white/70">({{ group.scientificName }})</span>
            </h3>
            <p class="text-white/80 mb-4">{{ group.description }}</p>
            
            <div class="grid md:grid-cols-2 gap-4 mt-4">
              <div
                v-for="(family, familyIndex) in group.families"
              ...
              >
                ...
                <div class="collapse-title text-white font-medium">
                  {{ family.name }}
                  <span class="text-sm font-normal text-white/60 ml-2">({{ family.scientificName }})</span>
                </div>
                <div class="collapse-content">
                  <p class="text-white/70 mb-3">{{ family.description }}</p>
                  <ul class="list-disc list-inside space-y-1 text-white/80 text-sm">
                    <li v-for="species in family.species" :key="species">
                      {{ species }}
                    </li>
                  </ul>
                </div>
